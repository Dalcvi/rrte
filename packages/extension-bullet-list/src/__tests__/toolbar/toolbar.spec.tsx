...
describe('Bullet list toolbar button', () => {
  ...
  it('should toggle bulletList on click', () => {
    ...
    expect(editor.toggleBulletList).toHaveBeenCalledTimes(2);
    expect(editor.run).toHaveBeenCalledTimes(1);
  });

  it('should disable when it cannot toggle bulletList', () => {
    const editor = new FakeEditor() as any;
  ...
  it('should not togglebulletList on click if disabled', () => {
    const editor = new FakeEditor() as any;
    editor.toggleBulletList = jest.fn().mockReturnValue(false);
