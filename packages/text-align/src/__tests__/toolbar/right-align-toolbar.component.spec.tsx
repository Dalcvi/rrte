import { RightAlignToolbarButton } from '../../toolbar';
...
describe('Text align right toolbar button', () => {
  afterEach(() => {
    jest.clearAllMocks();
  });
  it('should set text align right on click', () => {
    const editor = new FakeEditor() as any;

    render(<RightAlignToolbarButton.Button editor={editor} config={{ types: ['paragraph'] }} />);
  ...
  it('should disable when it cannot toggle text align right', () => {
    const editor = new FakeEditor() as any;
  ...
  it('should not text align right on click if disabled', () => {
    const editor = new FakeEditor() as any;
    editor.setTextAlign = jest.fn().mockReturnValue(false);
